...
export default function MontarCesta({ onBack }) {
  ...
  // --- 1. CARREGAR PRODUTOS E PREÇOS DO SUPABASE ---
  useEffect(() => {
    async function fetchStoreConfig() {
      try {
        ...
        if (data) {
          // Processa os produtos (active_products)
          const items = data.active_products || [];
          ...
          // Processa os preços
          setPrices(data.prices || {10:0, 15:0, 18:0});
        }
      } catch (e) {
    ...
    // Opcional: Atualizar a cada 30 segundos para pegar mudanças do admin em tempo real
    const intervalId = setInterval(fetchStoreConfig, 30000);
  ...
  // Atualiza mapa de quantidades quando os produtos carregam
  useEffect(() => {
    ...
    setQuantidades(prev => {
      // Mantém quantidades se o usuário já tiver clicado, apenas para produtos que ainda existem
  ...
  // Se não veio nada do banco ou array vazio, loja considerada "sem produtos"
  const storeClosed = !loading && produtosDisponiveis.length === 0;

  // --- LÓGICA DE CARRINHO (IGUAL AO ORIGINAL) ---
  function handleIncrement(prod) {
    setQuantidades(q => {
      const current = q[prod.id] || 0;
      if (current >= 3) return q;
  ...
  if (showRetirada) {
    return (
        <Retirada
            size={totalCount}
            cartItems={cart} // Passando o carrinho explicitamente
            onBack={() => {
        ...
        <div className="container">
          <div className="row justify-content-center">
            <div className="col-12 col-md-10 col-lg-8">
              ...
              {/* Loading / Lista */}
              {loading ? (
                  <div style={{ textAlign: 'center', color: '#fff', padding: 20 }}>Carregando produtos...</div>
              ) : (
              ...
              <div className="mc-cart-summary" style={{ marginTop: '14px', textAlign: 'center', color: 'rgba(255,255,255,0.95)' }}>
                ...
                {storeClosed ? (
                    <div style={{ fontSize: '0.95rem', opacity: 0.9 }}>Loja fechada (sem produtos cadastrados)</div>
                ) : finalPrice !== null ? (
